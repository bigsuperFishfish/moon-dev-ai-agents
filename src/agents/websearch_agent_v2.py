...
- **NEW: Strategy similarity detection with TF-IDF cosine similarity**
- **NEW: Content hash deduplication**
- Async processing capabilities
- Detailed logging and metrics tracking

Author: Moon Dev Trading Agents
Version: 2.1.0 (Similarity Detection Added)
Date: 2025-12-31
"""
...
# NEW: Similarity checking dependencies
try:
    ...
    cprint("⚠️  sklearn not installed, similarity detection disabled", "yellow")
...
class SearchConfig:
    ...
    # NEW: Similarity detection settings
...
DEDUPLICATION_LOG_CSV = DATA_DIR / "deduplication_log.csv"  # NEW
...
class StrategyData:
    ...
    similarity_score: float = 0.0  # NEW: Track similarity with existing strategies
    source_url: str = ""
...
class SearchCycleMetrics:
    ...
    strategies_skipped_similarity: int = 0  # NEW
    extraction_success_rate: float = 0.0
    average_quality_score: float = 0.0
    average_similarity_score: float = 0.0  # NEW
...
class CSVManager:
    ...
    def init_all():
        ...
        CSVManager.init_deduplication_log_csv()  # NEW
...
class SimilarityDetector:
    """Strategy similarity detection using TF-IDF + cosine similarity"""
    
    def __init__(self, threshold: float = SearchConfig.STRATEGY_SIMILARITY_THRESHOLD):
        self.threshold = threshold
        self.existing_strategies_cache: List[Dict[str, Any]] = []
        self._load_existing_strategies()
    
    def _load_existing_strategies(self):
        """Load existing strategies from markdown files"""
        try:
            existing_files = list(FINAL_STRATEGIES_DIR.glob("*.md"))
            logger.info(f"📚 Loading {len(existing_files)} existing strategies for similarity check")
            
            for filepath in existing_files:
                try:
                    with open(filepath, 'r', encoding='utf-8') as f:
                        content = f.read().lower()
                        self.existing_strategies_cache.append({
            ...
            logger.info(f"✅ Loaded {len(self.existing_strategies_cache)} strategies")
        except Exception as e:
            logger.warning(f"⚠️  Error loading existing strategies: {e}")
    
    def check_similarity(self, strategy_name: str, strategy_description: str) -> Tuple[bool, Optional[str], float]:
        """
        Check if similar strategy already exists
        Returns: (is_duplicate, best_match_filename, max_similarity)
        """
        if not SIMILARITY_AVAILABLE:
            return False, None, 0.0
        
        if not self.existing_strategies_cache:
            return False, None, 0.0
        ...
        try:
            ...
            for existing in self.existing_strategies_cache:
                ...
                if similarity > max_similarity:
                    ...
                    best_match = existing['filename']
            
            is_duplicate = max_similarity >= self.threshold
            
            if is_duplicate:
                logger.warning(f"⚠️  Duplicate detected: {strategy_name} (similarity: {max_similarity:.2%})")
                logger.warning(f"   Similar to: {best_match}")
    ...
    def add_strategy(self, strategy_name: str, description: str, filename: str):
        """Add new strategy to cache for future comparisons"""
        content = f"{strategy_name} {description}".lower()
        self.existing_strategies_cache.append({
            'filename': filename,
            'content': content
        })
...
class URLCache:
    ...
    def __init__(self, cache_file: Optional[Path] = None):
        ...
        self.content_hashes: set = self._load_content_hashes()  # NEW
    ...
    def _load_content_hashes(self) -> set:
        """Load content hashes from CSV (NEW)"""
        hashes = set()
        if SEARCH_RESULTS_CSV.exists():
            try:
                ...
                logger.debug(f"💾 Loaded {len(hashes)} content hashes")
            except Exception as e:
                logger.warning(f"⚠️  Failed to load content hashes: {e}")
        return hashes
    ...
    def is_content_duplicate(self, content_hash: str) -> bool:
        """Check if content hash already exists (NEW)"""
    ...
    def mark_processed(self, url: str, data: Dict[str, Any]):
        ...
        if 'content_hash' in data:
            self.content_hashes.add(data['content_hash'])  # NEW
        self._save_cache()
...
class LLMProvider:
    ...
    def generate_response(self, system_prompt: str, user_content: str,
        ...
        """Generate response using local Qwen via HPC server"""
    ...
    def fetch(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        while retry_count < SearchConfig.MAX_RETRIES:
            try:
                ...
                content_hash = hashlib.md5(text.encode()).hexdigest()[:12]  # First 12 chars
                
                logger.info(f"✅ Fetched {len(text)} chars (hash: {content_hash})")
...
class DataLogger:
    ...
    @staticmethod
    def log_deduplication(strategy_name: str, content_hash: str, similarity_score: float,
                         decision: str, reason: str, similar_to: str = ""):
        """Log deduplication decision (NEW)"""
        try:
...
class StrategyFileWriter:
    ...
    def save_strategy(strategy: StrategyData, search_query: str) -> Optional[str]:
        ...
        try:
            ...
            content += f"## Similarity Score\n\n{strategy.similarity_score:.1%}\n\n"  # NEW
            content += "---\n\n"
...
class SearchCycleOrchestrator:
    ...
    def __init__(self):
        ...
        self.similarity_detector = SimilarityDetector()  # NEW
        self.metrics = None
    ...
    def run_cycle(self, cycle_number: int) -> SearchCycleMetrics:
        ...
        for i, result in enumerate(results, 1):
            ...
            # Check if already processed (URL dedup)
            if url_cache.is_processed(result.url):
                logger.info(f"⏭️  Skipping (URL already processed)")
            ...
            # Check content hash dedup (NEW)
            if SearchConfig.CONTENT_HASH_DEDUP and url_cache.is_content_duplicate(page_data['content_hash']):
                logger.info(f"⏭️  Skipping (content hash duplicate: {page_data['content_hash']})")
                DataLogger.log_search_result(
                    cycle_number, query, result, "content_duplicate",
                    len(page_data['content']), page_data['content_hash']
                )
                continue
            ...
            # Save strategies with similarity detection (NEW)
            for strategy in strategies:
                # Check similarity with existing strategies
                is_duplicate, similar_to, similarity = self.similarity_detector.check_similarity(
                    strategy.strategy_name,
                    strategy.description
                )
                
                strategy.similarity_score = similarity
                
                if is_duplicate:
                    logger.warning(f"⚠️  Skipping duplicate: {strategy.strategy_name} (similarity: {similarity:.2%})")
                    ...
                    metrics.strategies_skipped_similarity += 1
                else:
                    ...
                    if saved:
                        ...
                        # Add to similarity detector cache
                        self.similarity_detector.add_strategy(
                            strategy.strategy_name,
                            strategy.description,
                            saved
                        )
                        metrics.strategies_extracted += 1
                        logger.info(f"✅ Saved with quality: {strategy.quality_score:.1%}, similarity: {similarity:.1%}")
        ...
        cprint(f"⚠️  Strategies skipped (similarity): {metrics.strategies_skipped_similarity}", "yellow")
...
def main():
    """Main entry point"""
    cprint("\n🌙 MOON DEV WEB SEARCH AGENT V2.1 (PRODUCTION + SIMILARITY)", "white", "on_magenta")
    cprint("🤖 Local LLM: Qwen2.5-7B (HPC Server 192.168.30.158:8000)", "cyan")
    ...
    cprint(f"🎯 Quality threshold: {SearchConfig.MIN_STRATEGY_QUALITY_SCORE:.2f} (saves ALL)", "yellow")
    cprint(f"📊 Similarity threshold: {SearchConfig.STRATEGY_SIMILARITY_THRESHOLD:.2f}", "yellow")
